...
/* ------------------------------ DATA ------------------------------ */
const GATE = 'https://purbanchaluniversity.edu.np/images/about/6f10d5ff48dc8ba065691da6baf4ec02.jpg';
...
const MOUS = [
  ['Jamia Millia Islamia University', 'India', 'https://jmi.ac.in/upload/mou/mou_jmi_purbanchal_university_nepal_2017february8.pdf'],
  ['Banaras Hindu University', 'India', 'https://bhu.ac.in/SiteAttachments/Academic%20Collaboration%20MOU2020314142950600.pdf'],
];

const OBJECTIVES = [
  ['Career-oriented education', 'Run professional and technical institutions, with study, research and teaching across disciplines, that develop the abilities and personalities of students, teachers and scholars.'],
  ['A competitive environment', 'Raise the level of competition in higher education by spreading knowledge and encouraging its efficient, effective use.'],
  ['Resources that improve quality', 'Draw on local, national and international resources to make academic programs better and more responsive to Nepal’s changing socio-economic needs.'],
  ['A model institution', 'Become a model of higher learning by correcting the academic weaknesses of both public and private institutions.'],
];

const FOCUS = ['Industry–Technology', 'Agriculture–Forestry', 'Environment, Rural & Cultural Subsistence', 'Sustainable Development'];

const LAND = [
  { key: 'forest', label: 'Productive forest', pct: '50%', ha: '272.5 ha', cells: 32, color: '#2f6b3f', note: 'Conservation and sustainable development' },
  { key: 'farm', label: 'High-income farming', pct: '12.5%', ha: '68 ha', cells: 8, color: '#c39a2b', note: 'Medicinal herbs and similar crops' },
  { key: 'agro', label: 'Agro-forestry', pct: '12.5%', ha: '68 ha', cells: 8, color: '#8db56a', note: 'Ecosystem management approach' },
  { key: 'infra', label: 'Buildings and facilities', pct: '25%', ha: '136 ha', cells: 16, color: '#1a1f3c', note: 'Academic, residential and support' },
];
...
/* ----------------------------- HELPERS ----------------------------- */
const focus = 'focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-pu-red';
...
/* Shared easing for a calm, editorial reveal */
const EASE = [0.22, 1, 0.36, 1];

/* Motion presets for sections — gentle fade + rise, played once */
const sectionMotion = {
...
/* Staggered reveal for paragraph blocks inside a section */
    ...
    >
      ...
      <ul className="divide-y divide-heading/10 border-y border-heading/10">
        {LAND.map((l, i) => (
          ...
          >
            <span aria-hidden className="mt-1.5 h-4 w-4 shrink-0 rounded-[3px]" style={{ backgroundColor: l.color }} />
            <div className="flex-1">
              <p className="font-semibold text-heading">{l.label}</p>
              <p className="text-sm text-body/70">{l.note}</p>
            </div>
            <p className="text-right">
              <span className="block text-xl font-bold tabular-nums text-heading font-heading">{l.pct}</span>
...
export default function AboutPu() {
  ...
  /* When reduced motion is on, sections render without animation */
  const S = reduce
  ...
  return (
    ...
    <main className="bg-white">
      ...
      {/* Just breadcrumb + page title. No hero, no big image, no fact strip. */}
      {/* <div className="border-b border-heading/10">
        <div className="container-x py-8 md:py-10">
          <motion.nav
            aria-label="Breadcrumb"
            {...B()}
            className="text-sm text-body/60"
          >
            <Link to="/" className={`hover:text-pu-red ${focus}`}>Home</Link>
            <span aria-hidden className="mx-2 text-body/25">/</span>
            <span className="text-body/60">About Us</span>
            <span aria-hidden className="mx-2 text-body/25">/</span>
            <span aria-current="page" className="font-medium text-heading">Purbanchal University</span>
          </motion.nav>
          ...
          >
            <h1 className="text-2xl font-bold tracking-tight text-heading font-heading md:text-3xl">
              About Purbanchal University
            </h1>
            <span lang="ne" className="text-sm font-medium text-pu-red md:text-base">
      ...
      <div className="container-x grid gap-16 py-16 md:py-20 lg:grid-cols-[200px_1fr] lg:gap-20">
            ...
            <ul className="mt-5 border-l border-heading/12">
              {TOC.map((t) => (
                <li key={t.id}>
                  ...
                  >
                    <span className={`text-[10px] tabular-nums ${active === t.id ? 'text-pu-red' : 'text-body/35'}`}>
                      {t.num}
                    </span>
                    {t.label}
                  </Link>
        ...
        <div className="max-w-[46rem] space-y-24 md:space-y-28">
          ...
          <motion.section id="overview" className="scroll-mt-28" {...S}>
            ...
            >
              PU was created to decentralise higher education and open it to regions of
              Nepal that had few options.
            </motion.p>
            ...
            <motion.div {...B(0.18)} className={`mt-8 space-y-5 ${body}`}>
              <p>
                For urban and rural families alike, it has become a place to earn a degree
                without leaving the country. Fees are kept affordable, and the campus,
                spread over 545 hectares in rural Koshi Province, gives students room to
                learn and grow.
              </p>
              <p>
                As a fully autonomous public university, PU sets its own policies and
                programs around the needs of the communities it serves. It works to
                preserve knowledge, protect free enquiry, and support development that
            ...
            >
              ...
              <p className={`mt-3 ${body}`}>
                PU is authorised to run the MBBS program. A three-way agreement lets
                students train at Koshi Hospital in Biratnagar, gaining clinical
              ...
              <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-stretch">
                ...
                <p className="flex flex-1 items-center bg-pu-dark px-5 py-4 text-sm font-semibold text-white">
                  Koshi Hospital, Biratnagar — teaching hospital for the MBBS program
                </p>
              </div>
            </motion.div>
          </motion.section>
          ...
          <motion.section id="partnerships" className="scroll-mt-28" {...S}>
            <SectionHeader num="02" title="Where PU sits among Nepal’s universities" />

            <motion.div {...B(0.05)} className={`space-y-5 ${body}`}>
              <p>
                Nepal has 19 universities: 11 established by the government, nine
                provincial, and autonomous academies such as the B.P. Koirala Institute
                of Health Sciences (BPKIHS).
              </p>
              <p>
                PU is governed by the Purbanchal University Act, so its structure differs
                from that of Tribhuvan University and Nepal Sanskrit University. It relies
                mostly on income it generates itself, chiefly student and affiliation
                fees, with minimal government support. Paying for its operations is a
                constant challenge, which is why PU looks to partnerships with
                universities, institutions and research centres, at home and abroad.
              </p>
            </motion.div>
          ...
          <motion.section id="vision" className="scroll-mt-28" {...S}>
            <div className="bg-pu-dark p-8 text-white md:p-12">
              ...
              >
                “To develop this university as the model in fulfilling the national
              ...
              >
                <p>
                  The aim is to help with Nepal’s pressing problems: falling productivity
                  in industry and agriculture, population growth and unemployment,
                  unsustainable development, the loss of natural resources, and
                  environmental decline.
                </p>
                <p>
                  Although a public university, PU sees itself as a bridge between public
                  and private institutions, offering affordable, worthwhile education
                  while staying self-sustaining and well governed. Nearly free public
                  universities on one side and very high private fees on the other have
                  opened gaps in the system. PU’s central challenge is to close them.
                </p>
              </motion.div>
              ...
              >
                {FOCUS.map((f) => (
                  <li key={f} className="bg-pu-dark px-5 py-4 text-sm font-medium">
                    {f}
                  </li>
                ))}
              </motion.ul>
          ...
          <motion.section id="campus" className="scroll-mt-28" {...S}>
            ...
            <motion.div {...B(0.05)} className={`space-y-5 ${body}`}>
              <p>
                The Government of Nepal gave PU 545 hectares of degraded forest land in
                Gothgaun, Morang, about 30 kilometres from Biratnagar and close to the
                East–West Highway. Under the Master Plan, PU has built a central academic
                building for the medical college, a central library, and a building for
                the proposed Peace Research Centre.
              </p>
              <p>
                Because the land was degraded, most of it is set aside to recover and
                earn. Each square below is about 8.5 hectares. Hover a row to find it on
                the grid.
              </p>
            </motion.div>
            ...
            <motion.p {...B(0.1)} className={`mt-3 ${body}`}>
              PU is developing its facilities with limited resources, in two planned
              phases.
            </motion.p>
            ...
            <ol className="mt-8 space-y-10 border-l border-heading/15 pl-8">
              <motion.li {...B(0.15)} className="relative">
                ...
                <p className={`mt-2 ${body}`}>
                  About Rs. 240 million was invested in construction at Gothgaun. Built
                  in this phase:
                </p>
